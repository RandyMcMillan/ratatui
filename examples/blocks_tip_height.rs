...
fn draw(frame: &mut Frame) {
    ...
    let mut height = mempool_space::api::api(blocks_tip_height, "");
    let blocks_tip_hash = &String::from("blocks_tip_hash");
    let mut hash = mempool_space::api::api(blocks_tip_hash, &height);
    let block = &String::from("block");
    let mut block_data = mempool_space::api::api(block, &hash);
    // title
    frame.render_widget(
        Paragraph::new(vec![
            Line::from(format!("{}:{}",height.clone(),hash.clone())).alignment(ratatui::layout::Alignment::Left),
            Line::from(format!("{}",block_data.clone())).alignment(ratatui::layout::Alignment::Left),
